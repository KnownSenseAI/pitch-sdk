...
| [Canonical speech render](https://github.com/KnownSenseAI/pitch-sdk/blob/main/examples/canonical-speech-render.ts) | `PITCH_PLACE_ID`, `PITCH_PLACE_REVISION`, `PITCH_PLACE_DISPLAY_TEXT`, `PITCH_VOICE_PROFILE_VERSION`, `PITCH_SPEECH_VOICE`; optional `PITCH_SPEECH_LOCALE`, `PITCH_SPEECH_RECIPE`, `PITCH_SPEECH_RECIPE_VERSION` | Resolve an approved place name, submit an idempotent utterance render, poll the durable operation, acknowledge its checksum, and obtain a managed delivery asset. |
